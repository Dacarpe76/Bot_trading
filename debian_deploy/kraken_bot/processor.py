...
class StrategyProcessor:
    def __init__(self):
        ...
        # 3. Aggressive (No Limit)
        w3 = PaperWallet("Aggressive", initial_balance=config.INITIAL_BALANCE, capital_limit_pct=config.CAPITAL_LIMIT_AGGRESSIVE)
        self.strategies['Aggressive'] = strategies.StrategyAggressive(w3)
        self.strategies['Aggressive'].on_event = self.handle_strategy_event
        
        # 4. AggrCent (No Limit)
        w4 = PaperWallet("AggrCent", initial_balance=config.INITIAL_BALANCE, capital_limit_pct=config.CAPITAL_LIMIT_AGGRESSIVE)
        self.strategies['AggrCent'] = strategies.StrategyAggressiveCent(w4)
        self.strategies['AggrCent'].on_event = self.handle_strategy_event
        
        # 5. NetScalp (Fixed Net + Safety)
        w5 = PaperWallet("NetScalp", initial_balance=config.INITIAL_BALANCE, capital_limit_pct=config.CAPITAL_LIMIT_PCT)
    ...
    def process_trade(self, symbol, trade):
        try:
            ...
            # We need latest indicators. They are in state['indicators'] (calculated at last candle close).
            # Ideally we recalculate indicators on tick? No, too heavy. Use last closed candle indicators.
            # Strategy checks trailing stop based on current Price + stored ATR.
            
            current_indicators = state['indicators']
